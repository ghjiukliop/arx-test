...
-- Hàm để join chế độ dựa trên tên chế độ
local function joinMode(mode)
    if mode == "Story" then
        joinMap() -- Gọi hàm join Story
    elseif mode == "Ranger Stage" then
        joinRangerStage() -- Gọi hàm join Ranger Stage
    elseif mode == "Boss Event" then
        joinBossEvent() -- Gọi hàm join Boss Event
    elseif mode == "Challenge" then
        joinChallenge() -- Gọi hàm join Challenge
    elseif mode == "Easter Egg" then
        joinEasterEggEvent() -- Gọi hàm join Easter Egg
    else
        print("Không tìm thấy chế độ: " .. tostring(mode))
    end
end

-- Vòng lặp tự động join theo thứ tự ưu tiên
spawn(function()
    while wait(5) do -- Kiểm tra mỗi 5 giây
        if not isPlayerInMap() then
            ...
            print("Đang ở trong map, chờ rời khỏi map để tiếp tục join.")
        end
    end
end)
